...
(function () {
  ...
  async function resendOtp() {
    ...
    try {
      const { error } = await getClient().auth.signInWithOtp({
        phone: state.phone,
        options: { shouldCreateUser: true, data: { full_name: state.name || null, shop_name: state.shop || null, source: 'qarzniuz-web' } }
      });
      if (error) throw error;
  ...
  function pinPanel() {
    ...
    panel.innerHTML = `
      <div style="width:min(380px,100%);text-align:center;font-family:inherit;">
        <div style="font-size:24px;font-weight:800;margin-bottom:8px;">QarzniUz</div>
        <div id="qarzniuz-pin-title" style="font-size:18px;font-weight:700;margin-bottom:8px;">PIN-kod yarating</div>
        <div id="qarzniuz-pin-hint" style="font-size:13px;color:#64748b;margin-bottom:18px;">4 xonali PIN-kod kiriting</div>
        <div id="qarzniuz-pin-dots" style="font-size:28px;letter-spacing:10px;min-height:42px;margin-bottom:16px;">○ ○ ○ ○</div>
        <input id="qarzniuz-pin-input" inputmode="numeric" autocomplete="off" maxlength="4" type="password" style="width:100%;box-sizing:border-box;text-align:center;font-size:26px;letter-spacing:14px;padding:12px;border:1px solid #cbd5e1;border-radius:12px;">
        <button id="qarzniuz-pin-btn" type="button" class="btn" style="width:100%;margin-top:12px;">Davom etish</button>
        <div id="qarzniuz-pin-error" style="min-height:20px;margin-top:10px;color:#b91c1c;font-size:13px;"></div>
      </div>`;
  ...
  async function setupOrVerifyPin(user) {
    ...
    let storedHash = meta.qarzniuz_pin_hash || null;
    let confirming = false;

    function resetInput() { input.value = ''; input.focus(); }
    function render() {
      title.textContent = storedHash ? 'PIN-kodni kiriting' : (confirming ? 'PIN-kodni tasdiqlang' : 'PIN-kod yarating');
      hint.textContent = storedHash ? 'QarzniUz hisobingizni ochish uchun 4 xonali PIN-kodni kiriting.' : (confirming ? 'Xuddi shu 4 xonali PIN-kodni yana kiriting.' : 'Keyingi kirishlarda SMS o‘rniga PIN-koddan foydalanasiz.');
      btn.textContent = storedHash ? 'Kirish' : (confirming ? 'PIN-kodni saqlash' : 'Davom etish');
    }
    render();
    resetInput();

    return new Promise(resolve => {
      const finish = () => { panel.remove(); sessionStorage.setItem(PIN_UNLOCK_KEY, '1'); resolve(); };
      btn.onclick = async () => {
        ...
        try {
          ...
          if (!storedHash) {
            ...
            if (first !== await hashPin(input.value)) return;
            const { data, error } = await getClient().auth.updateUser({ data: { qarzniuz_pin_hash: hash } });
            if (error) throw error;
            meta = data.user?.user_metadata || meta;
            storedHash = hash;
            finish();
          } else {
            if (hash !== storedHash) { errorEl.textContent = 'PIN-kod noto‘g‘ri.'; resetInput(); return; }
            finish();
          }
        } catch (e) {
          console.error('QarzniUz PIN error:', e);
          errorEl.textContent = 'PIN-kodni saqlash/tekshirishda xatolik. Qayta urinib ko‘ring.';
  ...
  async function verifyOtp() {
    ...
    try {
      ...
      setStatus('Tasdiqlandi. PIN-kod tekshirilmoqda...', true);
      await setupOrVerifyPin(data.user);
  ...
  function boot() {
    try { getClient(); installAuthHandler(); }
    catch (error) { console.error('QarzniUz Auth boot failed:', error); setStatus('Autentifikatsiya ishga tushmadi: ' + (error?.message || 'noma’lum xatolik'), false); }
  }
