(function(){
  ...
  function showMarketing(type, label){
    ...
    const usersHtml=rows.slice().sort((a,b)=>new Date(dateValue(b)||0)-new Date(dateValue(a)||0)).map(u=>`<div class="row" style="cursor:pointer" onclick="showUserByRef('${esc(userIdOf(u))}')"><span><b>${esc(u.name||'—')}</b><br><span class="muted small">${esc(u.phone||'—')} · ${esc(u.shop_name||u.shopName||'—')}</span></span>${statusPill(statusOf(u))}</div>`).join('');
  ...
  function enhanceMarketing(){
    ...
    if(sourceBox){
      ...
      if(sourceBox.children.length) sourceBox.insertAdjacentHTML('beforeend','<div class="marketingHint">Manbani bosib, foydalanuvchilar va konversiya tafsilotlarini ko‘ring.</div>');
    }
    ...
    if(campaignBox){
      ...
      if(campaignBox.children.length) campaignBox.insertAdjacentHTML('beforeend','<div class="marketingHint">Kampaniyani bosib, natijalarni ko‘ring.</div>');
    }
  }
